...
@Component({
  ...
  templateUrl: './second-direct.component.html',
  standalone: false,
  styles: [`
...
export class SecondDirectComponent implements OnInit, OnDestroy {
  ...
  private focusStep(step: Step): void {
    setTimeout(() => {
      (document.getElementById(`fld-d-${step}`) as HTMLElement | null)?.focus();
    }, 40);
  }
  ...
  lookupTicket(): void {
    ...
    this.api.getTicketByNumber(q, this.siteId)
      ...
      .subscribe({
        next: t => {
          if (t.status !== WeighmentStatus.PendingSecondWeight) {
            this.lookupError = t.status === WeighmentStatus.Completed
              ? 'This ticket has already been completed.'
              : 'This ticket is cancelled and cannot be completed.';
            this.cdr.markForCheck();
            return;
          }
          this.ticket = t;
          this.totalCharges = t.totalCharges;
          this.amountPaid = t.totalCharges;
          this.currentStepIndex = 1;
          this.focusStep('secondWeight');
          this.cdr.markForCheck();
        },
        error: (err: HttpErrorResponse) => {
          if (err.status === 401) {
            this.auth.logout(false); // logout already navigates to /auth/login
          } else if (err.status === 404) {
            this.lookupError = 'Ticket not found or not permitted.';
          } else {
            this.lookupError = err?.error?.message || err?.error?.title || 'Failed to load ticket. Please try again.';
          }
          this.cdr.markForCheck();
        }
      });
  ...
  save(): void {
    if (this.saving || !this.ticket) return;
    ...
    this.api.secondWeighment(this.siteId, {
      ...
      .subscribe({
        next: res => {
          ...
          this.printCount = 0;
          this.cdr.markForCheck();
          setTimeout(() => (document.getElementById('sd-print-btn') as HTMLElement)?.focus(), 60);
        },
        error: err => {
